...
function savePowerMode(){
  ...
  postForm('/api/setPowerMode', { enabled: enabled ? 'true' : 'false' })
    .then(() => postForm('/api/setDeepSleep', { idleSec }))
...
function saveDisplayMode(){
  ...
  postForm('/api/setDisplayMode', { enabled })
    .then(()=> postForm('/api/setDisplayOff', { sec }))
    .then(()=>{ showToast(wifiIsEn() ? 'Saved' : 'Збережено'); updateStatus(); })
    .catch(()=> showToast(wifiIsEn() ? 'Save error' : 'Помилка збереження'));
}
